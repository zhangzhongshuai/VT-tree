...
//  Box location;
  ...
  Tracks() = default;
  Tracks(int framestart, int frameend, int trackid) :framestart(framestart), frameend(frameend), trackid(trackid)
  {
  }
};
...
//      node->values.emplace_back(Tracks { cur.front().frameIndex, cur.back().frameIndex, trackid, box });
        node ->values.insert(Tracks { cur.front().frameIndex, cur.back().frameIndex, trackid});
    };
...
//        }
        if(box.left <= queryBox.left && box.top <= queryBox.top && box.left + box.width >= queryBox.left + queryBox.width && box.top + box.height >= queryBox.top + queryBox.height){
	        int xxx = 0;
          for(int i = 0; i < 5; i++ ){
            int stt = i * 1000 + xxx, ett = stt + 1000;
            node->values.query(stt,ett, values);
            xxx = 1000;
          }
    ...
    void queryT(const Box & queryBox, int st, int et, std::vector<Tracks>& ans) {
        queryT(mRoot.get(), queryBox, st, et, mBox, ans);
    }
private:
  ///阈值
  static constexpr auto Threshold = std::size_t(3);
  ///四叉树的最大深度
  static constexpr auto MaxDepth = std::size_t(6);

  static constexpr std::size_t QPlaneN = 1 << MaxDepth; // 平面四叉树的边长
  ...
  void insert(Node* node, const Box& box, std::vector<DetectorState>& track, int& cnt)
  {
    ...
    if (isLeaf(node)) {
      ++cnt;
      node->values.insert(Tracks(preprocess.front().frameIndex, preprocess.back().frameIndex, trackid));
    }
    else {
